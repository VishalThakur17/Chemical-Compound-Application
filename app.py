...
aromatic_model = joblib.load('model/aromatic_rings_rf_model.pkl')
rotatable_model = joblib.load('model/rotatable_bonds_rf_model.pkl')
qed_model = joblib.load('model/qed_weighted_rf_model.pkl')
heavy_model = joblib.load('model/heavy_atoms_rf_model.pkl')
hba_model = joblib.load('model/hba_lipinski_rf_model.pkl')
cxlogd_model = joblib.load('model/cx_logd_rf_model.pkl')
    ...
    if request.method == 'POST':
        ...
        if features:
            ...
            predictions = {
                ...
                'CxLogD': float(cxlogd_model.predict(features)[0]),
                'Molecular Weight': float(mw_model.predict(features)[0]),
                'Polar Surface Area': float(psa_model.predict(features)[0]),
                'Aromatic Rings': float(aromatic_model.predict(features)[0]),
                'Rotatable Bonds': float(rotatable_model.predict(features)[0]),
                'QED Weighted': float(qed_model.predict(features)[0]),
                'Heavy Atoms': float(heavy_model.predict(features)[0]),
                'HBA Lipinski': float(hba_model.predict(features)[0]),
